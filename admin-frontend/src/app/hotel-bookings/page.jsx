    ...
    <AuthGuard>
      <div className="admin-layout">
        ...
        <div className="admin-main">
          <Header title="Hotel Bookings" subtitle="All hotel reservations across properties" />
          <Inner />
        </div>
      </div>
    </AuthGuard>
...
function Inner() {
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    api.adminHotelBookingsList({ from: range.from, to: range.to })
      .then((res) => {
  ...
  return (
    <div className="dashboard-content">
      ...
      {!loading && !error && bookings.length > 0 && (
        <div className="fin-summary">
          ...
          <div className="fin-card payout">
            <span className="fin-label">Vendor payout</span>
            <span className="fin-value">{inr(totals.payout)}</span>
            <span className="fin-sub">Owed to hotels</span>
          </div>
        </div>
      )}
      ...
      <div className="table-card">
        ...
        {!loading && !error && bookings.length === 0 && (
          <div className="state">No hotel bookings in {rangeLabel.toLowerCase()}.</div>
        )}
              ...
              <tbody>
                {bookings.map((b) => (
                  <tr key={b.id}>
                    ...
                    <td>{b.hotel_name}<div className="muted small">{b.city}</div></td>
                    <td>{b.room_type}</td>
                    <td className="muted">{b.check_in}</td>
                    <td className="muted">{b.check_out}</td>
                    <td className="muted">{b.nights}</td>
                    <td className="muted">{b.guests}</td>
