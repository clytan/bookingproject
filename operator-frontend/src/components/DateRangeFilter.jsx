...
const startOfYear  = () => { const d = new Date(); return iso(new Date(d.getFullYear(), 0, 1)); };
...
export const defaultRange = () => ({ ...RANGE_PRESETS[2].build(), preset: '30d' });
  ...
  return (
    <div className="drf">
      <div className="drf-chips">
        ...
        ><FaCalendarAlt /> Custom</button>
      </div>

      {(customOpen || value.preset === 'custom') && (
        <div className="drf-custom">
          <label className="drf-field"><span>From</span><input type="date" value={value.from} onChange={setField('from')} /></label>
          <label className="drf-field"><span>To</span><input type="date" value={value.to} onChange={setField('to')} min={value.from || undefined} /></label>
        </div>
      )}
    </div>
  );
