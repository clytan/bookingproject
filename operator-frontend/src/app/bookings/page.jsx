    ...
    <AuthGuard>
      <div className="m-layout">
        ...
        <div className="m-main">
          <Header title="Bookings" subtitle="All reservations across your activities" />
          <Inner />
        </div>
      </div>
    </AuthGuard>
...
function Inner() {
  ...
  const load = () => {
    setLoading(true);
    api.bookingsList({ status, activity_id: activityFilter || undefined, from: range.from, to: range.to })
      .then((res) => {
        setItems(res.bookings || []);
        setTotals(res.totals || { gross: 0, admin_share: 0, payout: 0 });
        setCommission(res.commission_percent || 0);
      })
      .catch((e) => setError(e.message))
      .finally(() => setLoading(false));
  };

  useEffect(() => { load(); }, [status, activityFilter, range.from, range.to]);

  const filtered = source ? items.filter((b) => b.booking_source === source) : items;

  const activities = useMemo(() => {
    const m = new Map();
    items.forEach((b) => { if (!m.has(b.activity_id)) m.set(b.activity_id, b.activity_name); });
    ...
    <div className="m-content">
      ...
      {!loading && !error && items.length > 0 && (
        <div className="m-fin">
          ...
          <div className="m-fin-card share">
            <span className="m-fin-label">COKALO commission ({commission}%)</span>
      ...
      <div className="filters">
        <div className="filter-group">
          ...
          <select value={activityFilter} onChange={(e) => setActivityFilter(Number(e.target.value))}>
        ...
        <div className="filter-group">
          ...
          <select value={source} onChange={(e) => setSource(e.target.value)}>
            ...
            <option value="operator">Walk-in</option>
          </select>
        </div>
      ...
      <div className="m-card-wrap">
        ...
        {!loading && !error && filtered.length > 0 && (
          <table className="m-table">
            <thead>
              <tr>
                <th>Code</th><th>Guest</th><th>Contact</th><th>Activity</th><th>Slot</th><th>Date</th>
                <th>Departure</th><th>Persons</th><th>Total</th><th>Commission</th><th>Your payout</th>
